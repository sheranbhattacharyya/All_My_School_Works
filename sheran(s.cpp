...
int admin_or_sales;                    //To know whether the user is admin or salesperson
...
class date
{
	...
	void putdate()                        // to show the date on the screen
	{
		cout<<d<<"/"<<m<<"."<<y;
	}
...
void subscriber::display()               //to display the details
{
	cout<<"\n\t\tName: 	"<<name;
...
void subscriber::ipbill_mode()              //bill receiving mode
{
	int choice5;
...
void subscriber::ipstatus()                 //status
{
	int status_check=0;
	int choice3;
	do
...
void subscriber::ippay_mode()           //mode of aying the bill
{
	int paymode_check;
	int choice2;
	do
...
void subscriber::ipfacilities()            //facilities that are available
{
	int fac_reqd;
...
int check_mobile_no(char* mob_no)
{
	...
	subscriber d;
	flag=0;
	ifstream mobilecheck("subscribe.dat",ios::binary);
	while(mobilecheck.read((char*)&d,sizeof(d)))
	{
		if(strcmp(d.net_mobile_no(),mob_no)==0)
		{
			flag=1;
		}
	}
